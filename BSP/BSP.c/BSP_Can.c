...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    ...
    /*判断是哪一个Can口传回的信息*/
    if(hcan->Instance == CAN1) Can_Type = 0;
    else Can_Type = 1;

    HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0,
                         &Can_Data[Can_Type].CAN_RxTypedef.CANx_RxHeader,
                         Can_Data[Can_Type].CAN_RxTypedef.CAN_RxMessage);

    Can_Export_Data[Can_Type].CAN_RxHeader = Can_Data[Can_Type].CAN_RxTypedef.CANx_RxHeader;
    memcpy(&Can_Export_Data[Can_Type].CANx_Export_RxMessage,
           Can_Data[Can_Type].CAN_RxTypedef.CAN_RxMessage,
           sizeof(uint8_t[8]));

    /*把接收数据发给接收队列	*/
    if(!Can_Type) xQueueSendToBackFromISR(CAN1_ReceiveHandle,
                                              &Can_Export_Data[Can_Type],
                                              &xHigherPriorityTask);
    else xQueueSendToBackFromISR(CAN2_ReceiveHandle,
                                     &Can_Export_Data[Can_Type],
                                     &xHigherPriorityTask);
